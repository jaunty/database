...
func (o *User) SFMinecraftAccounts(mods ...qm.QueryMod) minecraftAccountQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"minecraft_accounts\".\"sf\"=?", o.SF),
	)

	query := MinecraftAccounts(queryMods...)
	queries.SetFrom(query.Query, "\"minecraft_accounts\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"minecraft_accounts\".*"})
	}
...
func (userL) LoadSFMinecraftAccounts(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`minecraft_accounts`),
		qm.WhereIn(`minecraft_accounts.sf in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load minecraft_accounts")
	}

	var resultSlice []*MinecraftAccount
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice minecraft_accounts")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on minecraft_accounts")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for minecraft_accounts")
	}

	if singular {
		object.R.SFMinecraftAccounts = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &minecraftAccountR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.SF == foreign.SF {
				local.R.SFMinecraftAccounts = append(local.R.SFMinecraftAccounts, foreign)
				if foreign.R == nil {
					foreign.R = &minecraftAccountR{}
				}
...
// AddSFMinecraftAccounts adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.SFMinecraftAccounts.
// Sets related.R.SFUser appropriately.
func (o *User) AddSFMinecraftAccounts(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*MinecraftAccount) error {
	var err error
	...
	if o.R == nil {
		o.R = &userR{
			SFMinecraftAccounts: related,
		}
	} else {
		o.R.SFMinecraftAccounts = append(o.R.SFMinecraftAccounts, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &minecraftAccountR{
				SFUser: o,
			}
		} else {
			rel.R.SFUser = o
